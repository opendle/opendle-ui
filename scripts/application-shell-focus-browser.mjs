...
import {ApplicationShell, MobileNavigation, Button, Icon, Dialog, FormField, Panel, PanelContent, GraphWorkspace, GraphInspector, GraphViewport, GraphNode} from "./dist/index.js";
function Fixture(){
 const [mounted,mount]=useState(true), [long,setLong]=useState(false), [route,setRoute]=useState(0), [modal,setModal]=useState(false), [inspector,setInspector]=useState(false);
 const heading=useRef(null), opener=useRef(null);
 useLayoutEffect(()=>{if(route) heading.current?.focus()},[route]);
 window.fixture={mount, long:setLong, route:setRoute};
 ...
 return <ApplicationShell sidebar={<aside aria-label="Desktop sidebar">Desktop navigation</aside>} mobileNavigation={<MobileNavigation aria-label="Phone destinations" items={items} onSelect={()=>setRoute(route+1)}/> }>
 <section className="fixture-content"><h1 tabIndex={-1} ref={heading}>Page {route}</h1>
 ...
 <div className="fixture-gap"/><label>Tall content<textarea className="fixture-tall-control" defaultValue="Tall editable content"/></label><Button>Last control</Button></section></ApplicationShell>;
}
...
.fixture-content{display:flex;flex-direction:column;gap:1rem;padding:1rem}.fixture-gap{height:700px;flex:none}.fixture-local-gap{height:900px}.fixture-content .od-panel-content{padding:1rem}.fixture-content h1{margin:0}.fixture-tall-control{display:block;width:100%;height:1000px}
