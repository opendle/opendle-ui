function assertFiniteGraphValue(value, name) {
    if (!Number.isFinite(value)) {
        throw new Error(`${name} must be finite.`);
    }
}
function assertGraphSize(size, name) {
...
function orderedGraphRange(minimum, maximum, fallbackMinimum, fallbackMaximum, name) {
...
export function clampGraphPosition(position, bounds) {
    assertFiniteGraphValue(position.x, "Graph position x");
    assertFiniteGraphValue(position.y, "Graph position y");
    const xRange = orderedGraphRange(bounds.minX, bounds.maxX, bounds.minX, bounds.maxX, "Graph position x");
    const yRange = orderedGraphRange(bounds.minY, bounds.maxY, bounds.minY, bounds.maxY, "Graph position y");
    return {
...
export function moveGraphPosition(position, delta, bounds) {
    assertFiniteGraphValue(position.x, "Graph position x");
    assertFiniteGraphValue(position.y, "Graph position y");
...
export function clampGraphViewport(viewport, limits = {}) {
    ...
    const xRange = orderedGraphRange(limits.minX, limits.maxX, -1_000_000, 1_000_000, "Graph viewport x");
    const yRange = orderedGraphRange(limits.minY, limits.maxY, -1_000_000, 1_000_000, "Graph viewport y");
    const zoomRange = orderedGraphRange(limits.minZoom, limits.maxZoom, 0.1, 4, "Graph viewport zoom");
    if (zoomRange.min <= 0) {
...
export function graphViewportCenter(viewport, viewportSize, limits = {}) {
...
export function graphPositionAtViewportCenter(viewport, viewportSize, itemSize = { width: 0, height: 0 }, limits = {}) {
...
function centeredZoomRange(contentCenter, viewportExtent, panMinimum, panMaximum) {
...
export function zoomGraphViewportAtPoint(viewport, zoom, anchor, limits = {}) {
...
export function fitGraphViewport(content, viewportSize, options = {}) {
    ...
    if (padding < 0)
        throw new Error("Graph fit padding must not be negative.");
    ...
    const widthZoom = content.width === 0
        ? Number.POSITIVE_INFINITY
        : availableWidth / content.width;
    const heightZoom = content.height === 0
        ? Number.POSITIVE_INFINITY
        : availableHeight / content.height;
    const requestedZoom = Math.min(widthZoom, heightZoom);
    const zoomRange = orderedGraphRange(options.minZoom, options.maxZoom, 0.1, 4, "Graph viewport zoom");
    if (zoomRange.min <= 0) {
        throw new Error("Graph viewport zoom must be greater than zero.");
    }
    const xRange = orderedGraphRange(options.minX, options.maxX, -1_000_000, 1_000_000, "Graph viewport x");
    const yRange = orderedGraphRange(options.minY, options.maxY, -1_000_000, 1_000_000, "Graph viewport y");
    ...
    const centeredX = centeredZoomRange(contentCenterX, viewportSize.width, xRange.min, xRange.max);
    const centeredY = centeredZoomRange(contentCenterY, viewportSize.height, yRange.min, yRange.max);
    ...
    const sizeZoom = Number.isFinite(requestedZoom)
        ? requestedZoom
        : zoomRange.max;
    const zoom = centeredMinimum <= centeredMaximum
...
export function layoutLayeredDirectedGraph(items, options = {}) {
    ...
    const knownParentsById = new Map();
    const childrenById = new Map(ids.map((id) => [id, []]));
    const remainingParents = new Map();
    for (const id of ids) {
        const item = itemById.get(id);
        if (!item)
            throw new Error(`Graph item is not available: ${id}`);
    ...
    while (ready.length) {
        ...
        for (const id of current) {
            orderedIds.push(id);
            const parentDepth = (knownParentsById.get(id) ?? []).reduce((depth, parentId) => Math.max(depth, depthById.get(parentId) ?? 0), -1);
    ...
    const maxBreadthCount = Math.max(1, ...[...layers.values()].map((layer) => layer.length));
    ...
    const breadthExtent = padding * 2 +
        maxBreadthCount * breadthSize +
        (maxBreadthCount - 1) * breadthGap;
    const depthExtent = padding * 2 + (maxDepth + 1) * depthSize + maxDepth * depthGap;
    const positioned = new Map();
    for (const [depth, layer] of layers) {
        const layerBreadth = layer.length * breadthSize + (layer.length - 1) * breadthGap;
        const breadthStart = padding + (breadthExtent - padding * 2 - layerBreadth) / 2;
    ...
    const edges = orderedIds.flatMap((targetId) => (knownParentsById.get(targetId) ?? []).map((sourceId) => ({
    ...
    return {
        ...
        nodes: orderedIds.map((id) => {
            ...
            if (!node)
                throw new Error(`Graph item was not positioned: ${id}`);
            return node;
        }),
        edges,
    };
