...
export function useInspectorReachability(hostRef, selectedControlRef, active) {
    useLayoutEffect(() => {
        ...
        const keepReachable = () => {
            ...
            const overlayAtStart = inspectorBounds.left < viewportBounds.left + viewportBounds.width / 2;
...
function hasForegroundModal(inspector) {
    return [
        ...inspector.ownerDocument.querySelectorAll("dialog[open]"),
    ].some((dialog) => dialog !== inspector &&
        !dialog.contains(inspector) &&
        isModalDialog(dialog));
}
function focusInspector(inspector, heading) {
    if (hasForegroundModal(inspector))
        return;
...
function useGraphInspectorMode(inspectorRef, headingRef) {
    ...
    const deferredScrollRef = useRef(null);
        ...
        const updateMode = () => {
            ...
            for (const control of currentControls) {
                if (control.getClientRects().length > 0) {
                    controlsEnd = Math.max(controlsEnd, control.getBoundingClientRect().bottom - hostStart);
                }
            }
            const measuredEnd = `${String(controlsEnd)}px`;
            if (inspector.style.getPropertyValue("--od-graph-inspector-controls-end") !== measuredEnd) {
                inspector.style.setProperty("--od-graph-inspector-controls-end", measuredEnd);
            }
    ...
    useLayoutEffect(() => {
        ...
        const applyMode = () => {
            ...
            const scroll = deferredScrollRef.current ?? {
                ...
                bodyScrollTop: transition?.bodyScrollTop ??
            ...
            const previousFocus = isGraphControlElement(currentFocus) && inspector.contains(currentFocus)
                ? currentFocus
                : transition?.focus;
            ...
            const body = inspector.querySelector(".od-graph-inspector-body");
            if (body)
                body.scrollTop = scroll.bodyScrollTop;
        ...
        const observer = new MutationObserver(() => {
            ...
            if (foregroundIsOpen === foregroundWasOpen)
                return;
...
export function GraphInspector({ activationKey, title, eyebrow, icon, actions, onClose, onCancel, closeLabel = "Close inspector", closeDisabled = false, returnFocusRef: suppliedReturnFocusRef, tone = "neutral", children, className, tabIndex, "aria-label": ariaLabel, "aria-labelledby": ariaLabelledBy, ...props }) {
    const titleId = useId();
        ...
        return () => {
            ...
            queueMicrotask(() => {
                if (!inspector.isConnected &&
                    ...
                    restoreInspectorFocus(requestedReturnTarget?.isConnected
