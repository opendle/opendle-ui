...
export function CalendarBoard({ mode, items, weekDates, weekHours, today, selectedId, draggedId, dropDate, onSelect, onDragStart, onDragEnd, onAllowDrop, onClearDrop, onMove, currentTimeLabel, renderChannel }) {
    if (mode === "timeline") {
        ...
        return _jsx("div", { className: "calendar-timeline", "aria-label": "Canonical activity timeline", children: sortedItems.map((item) => _jsxs("button", { type: "button", "data-selected": selectedId === item.id, onClick: () => onSelect(item), children: [_jsxs("time", { dateTime: `${item.date}T${item.start}:00Z`, children: [item.date, _jsxs("span", { children: [item.start, " UTC"] })] }), _jsx("i", { "aria-hidden": "true" }), _jsxs("span", { children: [_jsxs("small", { children: [item.kind.replace("-", " "), " \u00B7 ", item.state] }), _jsx("strong", { children: item.title }), _jsx("p", { children: item.detail })] }), item.channel ? _jsx(ChannelMark, { channel: item.channel, renderChannel: renderChannel }) : null] }, item.id)) });
    }
    if (mode === "month") {
        ...
        return _jsxs("div", { className: "calendar-month", "data-calendar-week-count": "6", "aria-label": `${monthLabel} month view`, children: [["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day) => _jsx("div", { className: "calendar-month-weekday", children: day }, day)), days.map((day, index) => { const dayItems = items.filter((item) => item.date === day.date).sort((a, b) => a.start.localeCompare(b.start)); const past = day.date < today; return _jsxs("div", { className: "calendar-month-day", "data-calendar-week": Math.floor(index / 7) + 1, "data-drop-active": dropDate === day.date, "data-calendar-event-count": dayItems.length, "data-in-month": day.inMonth, "data-past": past, "data-today": day.date === today, "aria-label": `${formatter.format(parseDate(day.date))}, ${dayItems.length} ${dayItems.length === 1 ? "event" : "events"}`, onDragEnter: (event) => onAllowDrop(event, day.date), onDragOver: (event) => onAllowDrop(event, day.date), onDragLeave: onClearDrop, onDrop: (event) => onMove(event, day.date), children: [_jsxs("div", { className: "calendar-month-date", children: [_jsx("span", { children: day.day }), day.date === today ? _jsx("small", { children: "Today" }) : null, past && dayItems.length ? _jsx("span", { "aria-label": "Read-only", children: "\uD83D\uDD12" }) : null] }), _jsx("div", { className: "calendar-month-events", children: dayItems.map((item) => _jsx(EventButton, { compact: true, item: item, selected: selectedId === item.id, dragging: draggedId === item.id, onSelect: onSelect, onDragStart: onDragStart, onDragEnd: onDragEnd, renderChannel: renderChannel }, item.id)) })] }, day.date); })] });
    }
    const weekdayFormatter = new Intl.DateTimeFormat("en-GB", { weekday: "short", timeZone: "UTC" });
    return _jsx("div", { className: "calendar-week-scroll", children: _jsxs("div", { className: "calendar-week", "aria-label": "Calendar week", children: [_jsx("div", { className: "calendar-week-corner" }), weekDates.map((date) => _jsxs("div", { className: "calendar-day-heading", "data-today": date === today, children: [_jsx("span", { children: weekdayFormatter.format(parseDate(date)) }), _jsx("strong", { children: parseDate(date).getUTCDate() }), date === today ? _jsx("small", { children: "Today" }) : null] }, date)), _jsx("div", { className: "calendar-time-axis", "aria-hidden": "true", children: weekHours.map((hour) => _jsx("span", { children: hour }, hour)) }), weekDates.map((date) => { const dayItems = items.filter((item) => item.date === date).sort((a, b) => a.start.localeCompare(b.start)); const past = date < today; return _jsxs("div", { className: "calendar-day-column", "data-drop-active": dropDate === date, "data-past": past, "data-today": date === today, onDragEnter: (event) => onAllowDrop(event, date), onDragOver: (event) => onAllowDrop(event, date), onDragLeave: onClearDrop, onDrop: (event) => onMove(event, date), children: [_jsx("div", { className: "calendar-hour-lines", "aria-hidden": "true", children: weekHours.map((hour) => _jsx("i", {}, hour)) }), date === today ? _jsx("div", { className: "calendar-now-line", "aria-label": `Current time${currentTimeLabel ? `, ${currentTimeLabel}` : ""}`, children: _jsx("span", { children: currentTimeLabel ?? "Now" }) }) : null, dayItems.map((item) => _jsx(EventButton, { item: item, selected: selectedId === item.id, dragging: draggedId === item.id, onSelect: onSelect, onDragStart: onDragStart, onDragEnd: onDragEnd, renderChannel: renderChannel }, item.id)), past ? _jsx("span", { className: "calendar-past-label", children: "\uD83D\uDD12 Read-only" }) : null] }, date); })] }) });
}
