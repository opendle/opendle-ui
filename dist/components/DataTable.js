...
function useDataTableFocus(rowIds) {
...
function DataTableContent({ actions, actionsLabel, ariaLabel, columns, eligibleRowIds, expansion, localPendingActions, minimumWidth, onAction, onExpand, onSelect, renderContext, rowLabels, rows, selectedEligibleCount, selection, selectionMode, sort, tableBusy, tableId, totalColumnCount, }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: "od-data-table-scroll-controls", children: [_jsx("button", { "aria-label": `Scroll ${ariaLabel} left`, onClick: () => {
                            ...
                            scrollRef.current?.scrollBy({ behavior: "smooth", left: 320 });
                                                ...
                                                selection.onChange([...next]);
                                            } })) : (_jsx("span", { className: "od-visually-hidden", children: "Select" })) })) : null, expansion ? (_jsx("th", { className: "od-data-table-control-heading", scope: "col", children: _jsx("span", { className: "od-visually-hidden", children: "Details" }) })) : null, columns.map((column) => (_jsx("th", { "aria-sort": column.sortable && sort
...
function ActionControls({ actions, context, label, localPendingActions, onAction, rowLabel, }) {
    return (_jsxs("fieldset", { className: "od-data-table-actions", children: [_jsxs("legend", { className: "od-visually-hidden", children: [label, " for ", rowLabel] }), actions.map((action) => {
...
function actionLockKey(rowId, actionKey) {
    return JSON.stringify([rowId, actionKey]);
}
function releaseLoadLock(lockRef, mountedRef, setPending) {
    lockRef.current = false;
    if (mountedRef.current)
        setPending(false);
}
function releaseActionLock(lockKey, lockRef, mountedRef, setPending) {
