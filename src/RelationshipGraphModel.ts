...
export function assertRelationshipGraphModel(
  nodes: readonly RelationshipGraphModelNode[],
  ...
  for (const node of nodes) {
    const kind: unknown = node.kind ?? "node";
    if (kind !== "group" && kind !== "node" && kind !== "row") {
      throw new Error(
        `Relationship graph node ${node.id} has an invalid kind.`,
      );
    }
    if (kind === "group" && node.parentId !== undefined) {
      throw new Error(
        `Relationship graph group ${node.id} must not have a parent.`,
      );
    }
    if (kind === "row" && node.parentId === undefined) {
      throw new Error(
        `Relationship graph row ${node.id} must name its parent group.`,
      );
    }
    if (kind !== "row" && node.parentId !== undefined) {
      throw new Error(
        `Relationship graph node ${node.id} must not have a parent group.`,
      );
    }
    ...
    if (parent?.kind !== "group") {
      throw new Error(
        `Relationship graph row ${node.id} refers to an unknown group.`,
      );
    }
    if (parent.columnIndex !== node.columnIndex) {
      throw new Error(
        `Relationship graph row ${node.id} must use its parent group's column.`,
      );
    }
  }
  ...
  for (const relationship of relationships) {
    if (!relationship.id.trim()) {
      throw new Error(
        "A relationship graph relationship must have an identifier.",
      );
    }
    if (relationshipIds.has(relationship.id)) {
      throw new Error(
        `Relationship graph relationship identifiers must be unique: ${relationship.id}.`,
      );
    }
    ...
    if (!source || !target) {
      throw new Error(
        `Relationship graph relationship ${relationship.id} refers to an unknown node.`,
      );
    }
    if (target.columnIndex - source.columnIndex !== 1) {
      throw new Error(
        `Relationship graph relationship ${relationship.id} must connect adjacent columns from left to right.`,
      );
    }
    if (source.actionable === false || target.actionable === false) {
      throw new Error(
        `Relationship graph relationship ${relationship.id} must use actionable endpoints.`,
      );
    }
  }
}
...
export function relationshipGraphPath(
  activeId: string | null,
  nodes: readonly RelationshipGraphModelNode[],
  relationships: readonly RelationshipGraphModelRelationship[],
): RelationshipGraphPath {
  const active =
    activeId === null ? undefined : nodes.find((node) => node.id === activeId);
  if (!active) return { nodeIds: new Set(), relationshipIds: new Set() };
  ...
  for (
    let columnIndex = active.columnIndex;
    columnIndex > 0;
    columnIndex -= 1
  ) {
    const next = new Set<string>();
    for (const relationship of relationships) {
      if (!leftFrontier.has(relationship.targetId)) continue;
      relationshipIds.add(relationship.id);
  ...
  for (
    ...
    const next = new Set<string>();
    for (const relationship of relationships) {
      if (!rightFrontier.has(relationship.sourceId)) continue;
      relationshipIds.add(relationship.id);
...
export function relationshipGraphSearch(
  ...
  const directMatchIds = new Set(
    nodes
      .filter((node) =>
        normalizeSearch(node.searchValue).includes(normalizedQuery),
      )
      .map((node) => node.id),
  );
...
function normalizedIndex(index: number, columnSize: number) {
  return columnSize <= 1 ? 0 : index / (columnSize - 1);
}

/** Return the next node for the three-column keyboard contract. */
export function relationshipGraphKeyboardTarget(
  currentId: string,
  key: string,
  visibleNodes: readonly RelationshipGraphModelNode[],
  relationships: readonly RelationshipGraphModelRelationship[],
): string | null {
  const actionableNodes = visibleNodes.filter(
    (node) => node.actionable !== false,
  );
  ...
  if (key === "ArrowDown") {
    return (
      currentColumn[Math.min(currentColumn.length - 1, currentIndex + 1)]?.id ??
      null
    );
  }
  if (key === "Home") return currentColumn[0]?.id ?? null;
  if (key === "End") return currentColumn.at(-1)?.id ?? null;
  ...
  candidates.sort((left, right) => {
    ...
    const distance =
      ...
      Math.abs(
        normalizedIndex(rightIndex, targetColumn.length) - currentPosition,
      );
